...
> **Estado atual (2026-09-23):** fundação, onboarding/equipe, clientes, solicitações, CRM Kanban, Inbox, fila/eventos e Cotações entregues — fases 1–4, 6, 7, 9, 10, 16, quase toda a 8 e parte da 5 (ver §9). Produção: https://saas-turismo-ivory.vercel.app
...
| 6 | ✅ concluída | 2026-09-23 | CRUD de clientes; tags por agência (criação inline, sugestões, exibidas na lista); notas internas (só o autor, dono ou gerente apagam); preferências estruturadas por categoria (memória para a IA; o usuário não pode gravar como se fosse a IA). Viajantes nominais ficam para a fase de reservas |
...
| 8 | 🟡 quase concluída | 2026-09-23 | Kanban com arrastar e soltar (mouse, toque, teclado) + menu “Mover para”; cards com cliente, destino, datas, passageiros, orçamento, consultor e temperatura; lead score por regras (destino 10, datas 10, passageiros 10, orçamento 15, completa 15; regras de cotação/proposta/intenção entram nas fases 13/16/17); temperatura pelas faixas da agência; personalização de etapas (renomear, cor, ordem, criar/remover próprias). Tarefas entregues (migration 0012): página com Minhas/Equipe/Atrasadas/Concluídas, prazos no fuso da agência, notificação ao responsável e regra automática “Preparar cotação” quando a solicitação fica completa (idempotente por índice único). Pendentes: pesos do score configuráveis, Realtime no quadro |
| 9 | ✅ concluída | 2026-09-23 | migration 0011: canais (token do WhatsApp nunca selecionável por clientes), conversas, mensagens (idempotência por `external_message_id`), notificações; mensagens só via RPC; financeiro não lê conversas; troca de modo auditada; resposta humana assume a conversa (D10). Inbox em 3 colunas com Realtime, filtros, responsável, encerrar/reabrir; canal **Simulação** explícito para testar sem WhatsApp; sino de notificações real (“cliente respondeu”). 68/68 testes de banco no Supabase real |
| 10 | ✅ concluída | 2026-09-23 | D4/D5 revistos: fila em Postgres + **Supabase Cron (pg_cron/pg_net) chamando rota protegida na Vercel** a cada minuto (sem worker dedicado). Migration 0013: `jobs` (claim com SKIP LOCKED, retries com backoff exponencial + jitter, dead-letter, dedupe, reclaim de locks abandonados), `domain_events` (outbox gravado por triggers de deals, solicitações e mensagens), jobs recorrentes (despacho de eventos a cada minuto, tarefas atrasadas a cada 15 min), consumidor “viagem confirmada”. Cliente service-role restrito por lint. 99/99 testes no Supabase real. **Ativação depende de CRON_SECRET/SUPABASE_SERVICE_ROLE_KEY na Vercel e dos segredos no Vault (ENVIRONMENT §2.5)** |
| 16 | ✅ concluída | 2026-09-23 | Antecipada (dependia só da fase 8). Migration 0014: `quotes` (ligada ao deal; cliente e solicitação derivados pelo banco), `quote_options`, `quote_items` com `details` por tipo (voo, hospedagem, transfer, passeio, seguro, outro — união Zod). **Preço, totais e margem calculados pelo banco** a cada escrita (valores do cliente ignorados) e espelhados em `pricing.ts` (centavos inteiros, arredondamento bancário para markup/comissão em %). Desconto acima do valor da opção rejeitado; opções/itens travados fora de `draft`. Criar cotação → deal em “Em cotação”; marcar pronta → “Cotação pronta”, conclui a tarefa “Preparar cotação” e emite `quote.created`/`quote.ready`. Telas: lista com filtros, nova cotação (a partir do negócio ou da solicitação), montador com várias opções, duplicar opção, itens com formulário por tipo, resumo de totais e margem. Atendente não lê cotações (RLS); financeiro só lê; custo/margem só chegam ao navegador com `quotes.view_margin` (consultor: padrão sim). 25 testes de banco + 15 unitários |
| 11–15, 17–30 | ⏳ não iniciada | — | IA (11–14) aguarda escolha do modelo e chave; WhatsApp (15) aguarda credenciais da Meta; Propostas (17) pode começar pela página pública e snapshot, com envio por WhatsApp quando a fase 15 chegar |
...
- Visibilidade de margem para consultores é “configurável” (SECURITY.md); hoje vale o padrão (vê). O ajuste por agência entra com as configurações de propostas.
